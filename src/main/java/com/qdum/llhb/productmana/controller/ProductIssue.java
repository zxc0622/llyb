...
@ControllerBind(controllerKey="/proManage/issue")
public class ProductIssue extends Controller {
	...
	public void queryAudit(){
		...
		List<Record> auditList = Product.dao.findProIssue(sta,user.getId(),curDate,pageIndex,num);
	...
	public void searchAudit(){
		...
		String curDate = sf.format(date);
		if(searchModel==1){
			List<Record> auditList = Product.dao.findProByTypeFirstAndProNameAndIssue(sta,user.getId(), curDate, typeValue, proName, pageIndex, num);
			...
			List<Record> auditList = Product.dao.findProByTypeFirstAndIssue(sta,user.getId(), curDate, typeValue, pageIndex, num);
			List<Product> countAudit = Product.dao.findCountByTypeFirst(sta,user.getId(), typeValue);
			List<Map<String,Object>> mapList = mapList(auditList,countAudit,curPage,num);
			renderJson(mapList);
		}else if(searchModel==3){
			List<Record> auditList = Product.dao.findProByProNameAndIssue(sta,user.getId(), curDate, proName, pageIndex, num);
		...
		if(auditList != null && auditList.size() >0){
			for (Record e : auditList) {
				if(e!=null){
					...
					map.put("create_date",sf.format(e.getDate("create_date")));
