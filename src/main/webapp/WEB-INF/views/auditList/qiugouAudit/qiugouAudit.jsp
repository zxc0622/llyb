...
<div class="main_right">
    <div class="cur_position">当前位置>系统审核>固废求购审核</div>
    <div class="tab_main">
        <div class="tab_title">
            <span class="cur_tab"><a href="${ctx}/auditList/qiugouAudit?con=1">待审核</a></span>
            <span onclick=""><a href="${ctx}/auditList/qiugouAudit?con=2">已审核</a></span>
			<span onclick=""><a href="${ctx}/auditList/qiugouAudit?con=0">审核未通过</a></span>
        </div>
        <form action="">
        	<!--行业分类筛选-->
                    ...
                    <c:forEach items="${lists }" var="u">
                    	<tr>
	                       ...
	                       <c:if test="${empty u.pic1 && not empty u.pic2}">
	                        ...
	                        <c:if test="${con == 1 }">
		                        <td>
		                            <a class="tr_manage_1" href="${ctx}/auditList/qiugouAudit/checkDetails?id=${u.iid}" style="position:relative;color: #5EC520">&nbsp;<h6>审核</h6></a>
		                        </td>
                       		 </c:if>
                       		 <c:if test="${con ne 1 }">
            ...
            <div class="filp_box">
                ...
                <div class="filp">
				  ...
				  <c:set var="actionUrl" value="${ctx}/auditList/qiugouAudit/"/>
...
<script>
	...
	function search(){
		...
		window.location.href="${ctx}/auditList/qiugouAudit?leibie="+a+"&title="+t+"&con=${con}";
	}
	function reset(){
			...
			$.ajax({
				...
				success:function(data){
					if(data == false){
						...
						window.location.href="${ctx}/auditList/qiugouAudit?leibie=${leibie}&title=${title}&con=${con}";
					}
				}
			});
		}
