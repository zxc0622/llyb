...
<div class="main_right">
    <div class="cur_position">当前位置>系统审核>固废供应审核</div>
    <div class="tab_main">
        <div class="tab_title">
            <span onclick="parent.iframeGoToUrl('${ctx}/auditList/supplyAudit?con=1')">待审核</span>
            <span onclick="parent.iframeGoToUrl('${ctx}/auditList/supplyAudit?con=2')">已审核</span>
            <span onclick="parent.iframeGoToUrl('${ctx}/auditList/supplyAudit?con=0')">审核未通过</span>
        </div>
        <form action="">
        	<!--行业分类筛选-->
            <div class="tab_main_row">
                <select name="" id="leibie" onChange="" >
                    ...
                    <c:forEach items="${hy}" var="hy" >
		             	<option value="${hy.id }">${hy.name}</option>
					</c:forEach>
                </select>
                <input type="text" value="${title }"/>
                <input type="button" value="搜索" style="background-color:#5cb531; border:1px solid #5cb531; color:white;"/>
                <input type="button" value="重置"/>
                <input class="hide" onclick="passOrNot('Y','')" type="button" value="通过" />
				<input class="hide" onclick="parent.showText('请填写不通过的原因')" type="button" value="不通过" />
            </div>
            <!--筛选结果-->
                    ...
                    <c:forEach items="${lists }" var="u">
                    	<tr>
	                       ...
	                       <c:if test="${empty u.pic1 && not empty u.pic2}">
	                        ...
	                        <td>${u.info_title }</td>
	                        <td>${u.pname }-${u.pname }</td>
	                        <td>${fn:substring(u.time, 0, 10)}</td>
							<td>${u.uname}</td>
							<td class="auth"><span>${u.type}</span><span class="${u.pauth}"></span><span class="${u.cauth}"></span></td>
	                        <c:if test="${con == 1 }">
		                        <td>
		                            <a class="tr_manage_1" href="${ctx}/auditList/supplyAudit/audit?id=${u.iid}" style="position:relative;color: #5EC520">&nbsp;<h6>审核</h6></a>
		                        </td>
                       		 </c:if>
                       		 <c:if test="${con ne 1 }">
            ...
            <div class="filp_box">
                ...
                <div class="filp">
				  ...
				  <c:set var="actionUrl" value="${ctx}/auditList/supplyAudit/"/>
...
<script>
	...
	function search(){
		...
		window.location.href="${ctx}/auditList/supplyAudit?leibie="+a+"&title="+t+"&con=${con}";
	}
	function reset(){
