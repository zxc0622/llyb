...
<head>
	...
	<script type="text/javascript">
		...
		function hrefLb(obj,str){
			var arr=$("#type").val();
			if(str=='1'){
				window.location.href="${ctx}/sys/category/form?id="+obj+"&lbs="+arr+"";
			}else if(str=='2'){
				window.location.href="${ctx}/sys/category/form?create.parentId="+obj+"&lbs="+arr+"";
			}else if(str=='3'){
				window.location.href="${ctx}/sys/category/form?lbs="+arr+"";
			}else if(str=='4'){
				window.location.href="${ctx}/sys/category/delete?id="+obj+"&leibie="+$("#type").val()+"";
			}
        ...
        <div class="me_content_start">
           ...
           <%--  <tags:message content="${message}"/> --%>
            ...
            <div class="me_search_crisis_all mb10">
            	<div class="fl font_size_14 me_liebiao">类别列表：</div>
                <div class="fr">
                    <input type="button" onclick="hrefLb('','3')" class="me_button_two" id="me_four_pop_click" value="+&nbsp;新增类别">                  
                </div>
                <div class="cb"></div>
            </div>
            <div class="table_one">
            <table id="treeTable" width="100%" >
			 		...
			 		<c:if test="${fn:length(list) > 0}">   
                       <c:forEach items="${list}" var="category">
						<tr id="${category.id}" pId="${category.parentId ne 1?category.parentId:'0'}">
							<td>${category.name}</td>
							<td>${category.code}</td>
							<td>${category.sort}</td>
                        	<td>${category.remarks}</td>
							 <td class="color_00bfb0">
								...
								<a title="添加下级类别" class="cp" onclick="hrefLb(${category.id},'2')" href="javascript:void(0)">添加下级类别</a>
							</td>
						</tr>
					</c:forEach>
				</c:if> 
