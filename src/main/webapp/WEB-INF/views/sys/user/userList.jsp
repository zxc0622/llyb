...
<body>
        	...
        	<div class="me_change_all">
        		<a class="me_change_a cp fl me_change_befault" href="${ctx}/sys/user" id="mendian">账号管理</a>
            	<%--<a class="me_change_a1 cp fl " href="${ctx}/sys/user/student" id="cangku">学员账号管理</a>--%>
        	</div>
        <div class="me_content_start">
            ...
            <form id="searchForm"  action="${ctx}/sys/user" method="post">
            	 ...
            	 <div class="col-lg-4">
		            <div class="input-group">
		                <span class="input-group-addon me_search_span">用户类型：</span>
		             	<select id="type" name="type"  onchange="tex();">
		                    <option value="">请选择用户类型</option>
		                    <c:forEach items="${fns:getDictList('user_type')}" var="dict">
		                    	<%--<c:if test="${dict.value != 3}">--%>
								<option value="${dict.value }"<c:if test="${dict.value == page.map.type}">selected</c:if>>${dict.label}</option>
								<%--</c:if>--%>
							</c:forEach>
		            	</select>
		            </div>
            ...
            <div class="me_search_crisis_all mb10">
                ...
                <div class="fr">
                  ...
                  <input type="button" onclick="location.href='${ctx}/sys/user/form'" class="me_button_two mr5" id="me_four_pop_click" value="+&nbsp;新增账号">
                  </shiro:hasPermission>
                </div>
                <div class="cb"></div>
            </div>
            ...
            <table width="100%" >
                       ...
                       <shiro:hasPermission name="sys:user:edit"><th>操作</th></shiro:hasPermission></tr>
                        <c:if test="${page.count == 0}">
							<tr>
								<td colspan="7" align="center">暂无账号信息</td>
							</tr>
						</c:if>
					<c:if test="${page.count > 0}">   
                       <c:forEach items="${page.list}" var="user">
						<tr>
							...
							<td>${fns:getDictLabel(user.type,'user_type', '无')}</td>
							<td>${user.name}</td>
