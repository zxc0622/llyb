...
<%@ include file="/WEB-INF/views/Library/share/EnterpriseTop.jsp"%>
         <div class="qy_m_r">
                <div class="qy_m_r_div">
                    ...
                    <div class="qy_m_r_d_main">
                        <div class="jieshaopic">
                            <img src="${companyList.performImg}" alt="">
                        </div>
                        <div class="jieshaotxt">
                            <p>${fn:substring(companyList.shopDesc, 0, 258)}<c:if test="${fn:length(companyList.shopDesc)>258}">...</c:if></p>
                            <a style="float:right;margin-right:15px;" href="javascript:void(0)"onclick="go('${ctx}/homepage/introduction/${enterpriseId}')">查看详情>></a>
                        </div>
                    </div>
                </div>
                <div class="qy_m_r_div">
                    ...
                    <div class="qy_m_r_d_main">
                        <c:forEach items="${supplyList}" var="supplyList">
	                        ...
	                        <c:choose>
	                        	<c:when test="${supplyList.pic1!=null}">
	                        		<a href="${ctx}/mall/detail/${supplyList.id}"><img src="${supplyList.pic1}" alt=""></a>
	                        	</c:when>
	                        	<c:when test="${supplyList.pic2==null}">
	                        		<a href="${ctx}/mall/detail/${supplyList.id}"><img src="${supplyList.pic3}" alt=""></a>
	                        	</c:when>
	                        	<c:otherwise>
	                        		<a href="${ctx}/mall/detail/${supplyList.id}"><img src="${supplyList.pic2}" alt=""></a>
	                        	</c:otherwise>
	                        </c:choose>

	                            <p>${supplyList.proName}</p>
	                            <p>${supplyList.createDate}</p>
	                        </div>
                        </c:forEach>
                    </div>
                </div>
                <div class="qy_m_r_div">
                    <div class="qy_m_r_d_title">
                        <span>公司相册</span>
                        <a href="javascript:void(0)"onclick="go('${ctx}/homepage/image/${enterpriseId}')">MORE+</a>
                    </div>
                    <div class="qy_m_r_d_main">
