		...
		<div style="width: 700px; margin: 0 auto; border: 1px solid #008f01;">
			<div style="width: 600px; margin: 0 auto;">
				...
				<ul class="summary">
					...
					<li class="color_red">项目名称：${r.pro_name }</span>
					<li class="color_red">地址：${r.aname} ${r.aaname } ${r.ssaname}</li>
					<li class="color_red">类别：${r.hname }</li>
				</ul>
				<ul class="values" style="text-indent: 36px;">
					<c:forEach items="${rr }" var="r">
						<li>${r.Survey_detail_type_name }</li>
					</c:forEach>
				</ul>
				<div>
					<span style="font-size:14px;line-height:30px;">总金额 :${money.money }</span>
				</div>
				<div class="per-row color_red clearfix">
					<p class="per-row-p" style="font-size: 14px;display: block;">备注：</p>
					<p class="per-row-p" style="text-indent: 36px;">1、平台1个工作日内推荐专业检测公司与您取得联系并提供服务。</p>
				...
				<c:if test="${bs ne 1}">
					<div class="per-row clearfix">
						<p class="per-row-p" style="font-size: 16px; line-height: 35px; display: block;">您对报价结果:</p>
		    	...
		    	if(appr == undefined){
		    		...
		    		$.ajax({
		      			type : "POST",
		    				...
		    				data : "appr="+appr+"&eid=${r.id}&amount=${money.money }",
		    				dataType : 'text',
