...
<style type="text/css">
	...
	#hbxq tr td{border-bottom: 1px dashed #cccccc;}
	/*系统消息sys_m*/
...
<body>
	<div class="main">
		...
		<div class="main_right">
			...
			<div class="biddingprocess">
				...
				<div class="m_r_title">
					...
					<div style="float: right; text-align: center; line-height: 30px;">
						...
						</select> <a href="javascript:shaixuan();" class="shaixuan">筛 选</a>
					</div>
				</div>
				<!-- 需求查看记录 子标题栏end -->
				<!-- 需求查看记录 -->
								...
								<tbody class="req_list">
									<c:forEach items="${rs }" var="hb" varStatus="st">
										<tr class="t_tr_hbxq" id="${hb.id }">
											...
											<td>
												<c:if test="${hb.source_flag == 0}">大厅</c:if>
												<c:if test="${hb.source_flag == 1}">精准</c:if>
												<c:if test="${hb.source_flag == 2}">精准、大厅</c:if>
											</td>
											<td>${hb.trade_status }</td>
											<td style="text-align: center;">
												<c:if test="${hb.trade_status == '提交'}">
													<a style="width: 40px;height: 20px;line-height: 20px;display: inline-block;background-color: #5ec520;color: white;cursor: pointer;">是</a>
													<a style="width: 40px;height: 20px;line-height: 20px;display: inline-block;background-color: #ff5722;color: white;cursor: pointer;">否</a>
												</c:if>
											</td>
										</tr>
									</c:forEach>
			...
			<div class="save">
				<span class="end_title"> <a href="javascript:save();">保存</a>
				</span>
			</div>
		</div>
	</div>
