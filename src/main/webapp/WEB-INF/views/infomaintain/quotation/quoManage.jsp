...
<html>
    ...
    <body>
            ...
            <div class="main_right">
                <div class="m_r_position font_1">当前位置>行情中心>行情管理</div>
                <div class="tab_main">
                    <div>
                        <a href="javascript:goUrl('${ctx}/infomaintain/quotation/add')"class="btn new_tr">新增</a>
                    ...
                    <div>
                        <table class="zxgl_table">
                            ...
                            <tr>
                                <td style="width:45px">
                                    <input type="checkbox" value="${quo.id}">
                                </td>
                                <td style="width:45px"><fmt:parseNumber value='${quo.rownum}' type="number" integerOnly="true"/></td>
                                <td style="width:250px;"class="tmui-ellipsis">${quo.quo_title}</td>
                                <td style="width:89px;">${quo.sd_label}</td>
                                <td style="width:57px">${quo.sta eq '1'? '已发布':'草稿箱'}</td>
                                <td style="width:162px;">${fn:substring(quo.time,0,19)}</td>
                                <td style="width:92px;">
                    ...
                    <div class="filp_box">
                        <div class="filp">
			                ...
			                <c:set var="actionUrl" value="${ctx}/infomaintain/quotation/"/>
			                <c:set var="urlParas" value="?name=name"/>
        ...
        <script type="text/javascript">
          ...
          /* //上一页
        	$(".prev_page").click(function(){
        		...
        		if(curPage!=1){
        			curPage--;
        			location.href="${ctx}/infomaintain/quotation?pageNum="+curPage+"";
        		}
        	});
        	//下一页
        	...
        	function deleteSelected(ids){
        		if(ids.length>1){
        			$.ajax({
            			type:"post",
            			url:"${ctx}/infomaintain/quotation/delete",
            			data:"ids="+ids,
            			async:false,
            			success:function(data){
            				if(data==true){
            					goUrl("${ctx}/infomaintain/quotation");
            				}else{
        	...
        	//修改
        	function update(obj){
        		location.href="${ctx}/infomaintain/quotation/add?id="+$(obj).parents("tr").find("input").val();
        	}
        </script>
    </body>
</html>