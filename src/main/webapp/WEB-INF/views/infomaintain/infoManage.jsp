...
<html>
    ...
    <body>
            ...
            <div class="main_right">
                <div class="m_r_position font_1">当前位置>资讯维护>资讯管理</div>
                <div class="tab_main">
                    <div style="margin-bottom:5px;">
                    <form id="search_form" action="${ctx}/infomaintain" method="post">
                        <a href="javascript:goUrl('${ctx}/infomaintain/add')"class="btn new_tr bnt_short">新增</a>
                    	...
                    	<div style='float:right;'>
			                ...
			                <select class="info_type" name="type" id="info_type">
			                	<option value="">选择类别</option>
                                <c:forEach items="${dictList}" var="dict">
                                    <option value="${dict.value}" <c:if test="${type == dict.value}"> selected = selected</c:if> >${dict.label}</option>
                                </c:forEach>
	                        </select>
			
			                
			                <input class="info_title" placeholder="请输入文章名称" type="text" name="title" value="${title }" style="width:180px;"/>
                    ...
                    <div >
                        <table class="zxgl_table">
                            ...
                            <tr>
                                ...
                                <td style="width:250px;"class="tmui-ellipsis">${info.new_title}</td>
                                <td style="width:89px;">${info.sd_label}</td>
                                <td style="width:57px">${info.sta eq '1'? '已发布':'草稿箱'}</td>
                                <td style="width:162px;">${fn:substring(info.time,0,19)}</td>
                                <td style="width:100px;">${info.show_order}</td>
                                <td style="width:92px;">
                                    <a href="#" onclick="update(this)">修改</a>
                                    <a href="#" onclick="deleteOne(this)">删除</a>
                                </td>
                                <td style="width:60px;"><input type="button" value="置顶" onclick="showTop(${info.id})" style="width:43px; height:27px;color:#FFF; background-color:#5EC520" /></td>
                            </tr>
                            </c:forEach>
                        </table>
                    </div>
                    ...
                    <div class="filp_box">
                        <div class="filp">
			                ...
			                <c:set var="actionUrl" value="${ctx}/infomaintain/"/>
        ...
        <script type="text/javascript">
          ...
          /* //上一页
        	...
        	//修改
        	function update(obj){
        		location.href="${ctx}/infomaintain/add?id="+$(obj).parents("tr").find("input").val();
        	}
        	//置顶
        	function showTop(id){
        		$.ajax({
