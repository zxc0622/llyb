        ...
        <div class="tgpm_main_bottom clearfix">
            <div class="fl tgpm_mb_div">
                <span class="tgpm_mb_title">
                    起价：2000元${keyWord}供应排名推广
                </span>
                <div class="chujiajilu_div">
                    <div class="cjjl_d_title">
                        ${year}年${month}月份出价记录
                    </div>
                    <div class="hasnot_re">
                        暂无记录
                    ...
                    <ul>
                    <c:forEach items="${bidRecordList}" var="bidRecord">
                    	<c:if test="${bidRecord.rank_type eq 'S'}">
                        <li>${bidRecord.bid}元(<fmt:formatDate value="${bidRecord.create_date}" type="both" pattern="yyyy.MM.dd"/>) ${bidRecord.comp_name}</li>
                    	</c:if>
                    </c:forEach>
                    </ul>
                    <a href="javascript:void(0)" class="btn"style="display: block;margin:5px auto 0">我要出价</a>
                </div>
                <div class="lishi_chujiajilu_div">
                    <div class="lishi_cjjl_d_title">
                        <a href="javascript:go('${ctx}/spread/index/more?keyWord=${keyWord}&mid=0')">更多..</a>历史出价记录
                    </div>
                    <div class="hasnot_re">
                        暂无记录
                    </div>
                    ...
                    <c:forEach items="${bidRecordAllList}" var="bidRecord">
                    	...
                    	<c:if test="${bidRecord.rank_type eq 'S' && i<5}">
            ...
            <div class="fl tgpm_mb_div">
                ...
                <div class="chujiajilu_div">
                    ...
                    <ul>
                    <c:forEach items="${bidRecordList}" var="bidRecord">
                    	<c:if test="${bidRecord.rank_type eq 'D'}">
                        <li>${bidRecord.bid}元(<fmt:formatDate value="${bidRecord.create_date}" type="both" pattern="yyyy.MM.dd"/>) ${bidRecord.comp_name}</li>
                    	</c:if>
                    </c:forEach>
                    </ul>
                ...
                <div class="lishi_chujiajilu_div">
                    <div class="lishi_cjjl_d_title">
                        <a href="javascript:go('${ctx}/spread/index/more?keyWord=${keyWord}&mid=1')">更多..</a>历史出价记录
                    </div>
                    <div class="hasnot_re">
                        暂无记录
                    </div>
                    ...
                    <c:forEach items="${bidRecordAllList}" var="bidRecord">
                    	<c:set var="i" value="0"></c:set>
                    	<c:if test="${bidRecord.rank_type eq 'D' && i<5}">
                    	<c:set var="i" value="${i+1}"></c:set>
                        <li>${bidRecord.bid}元(<fmt:formatDate value="${bidRecord.create_date}" type="both" pattern="yyyy.MM.dd"/>) ${bidRecord.comp_name}</li>
                    	</c:if>
                    </c:forEach>
            ...
            <div class="fl tgpm_mb_div">
                ...
                <div class="chujiajilu_div">
                    ...
                    <ul>
                    <c:forEach items="${bidRecordList}" var="bidRecord">	
                    	<c:if test="${bidRecord.rank_type eq 'E'}">
                        <li>${bidRecord.bid}元(<fmt:formatDate value="${bidRecord.create_date}" type="both" pattern="yyyy.MM.dd"/>) ${bidRecord.comp_name}</li>
                    	</c:if>
                    </c:forEach>
                    </ul>
                ...
                <div class="lishi_chujiajilu_div">
                    <div class="lishi_cjjl_d_title">
                        <a href="javascript:go('${ctx}/spread/index/more?keyWord=${keyWord}&mid=2')">更多..</a>历史出价记录
                    </div>
                    <div class="hasnot_re">
                        暂无记录
                    </div>
                    ...
                    <c:forEach items="${bidRecordAllList}" var="bidRecord">
                    	<c:set var="i" value="0"></c:set>
                    	<c:if test="${bidRecord.rank_type eq 'E' && i<5}">
                    	<c:set var="i" value="${i+1}"></c:set>
                        <li>${bidRecord.bid}元(<fmt:formatDate value="${bidRecord.create_date}" type="both" pattern="yyyy.MM.dd"/>) ${bidRecord.comp_name}</li>
                    	</c:if>
                    </c:forEach>
...
$(function(){
    ...
    $(".chujiajilu_div a").click(function(){
    	showMess("请联系客服 ");
    });
});
function ulhaschild(obj){
	var ul=obj.find("ul");
	ul.each(function(){
		if($(this).has("li").length!=0)
		$(this).css("min-height","50px").siblings(".hasnot_re").hide();
	});
}
//不带确定的提示框
function showMess(str){
	...
	$(".pl_normal").show();
	var colse_mess = setTimeout(function(){
		$(".popumask").fadeOut();
