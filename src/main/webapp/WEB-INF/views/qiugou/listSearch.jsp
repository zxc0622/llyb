...
<style>
    ...
    .result_list .list_row{width:742px; height:131px; border:1px solid #ccc; padding:24px 10px; overflow:hidden; margin-bottom:6px;}
    ...
    <div class="header_middle" style="height: 145px;">
        ...
        <div class="header_search">
            <select >
                ...
                <option value="2" selected="selected">固废求购</option>
            </select>
								 	...
								 	<div class="level-div level-erji" id="${solid.id }">
				                       ...
				                       <ul>
										<c:forEach items="${solids}" var="solidChilds">
						                    <c:if test="${solidChilds.parent.id eq solid.id}">
						                    	<c:choose>
...
<script>
	...
	$(document).ready(function(){
	   ...
	   if('${typeId}' != "" && '${typeId}' != "null"){
		   ...
		   if('${searchtext}'){
			   $("#header_search").prev().val('${searchtext}');
			   list(1,'${searchtext}');
			   getSupplyNum();
		   }else{
			   list(1,'');
     ...
     //所在地切换
	   ...
	   $(".level-erji li").on("click",function(){
		...
		/* $("#header_search").on("click",function(){
			...
			if(s == '1'){
				location.href="${ctx}/solidsupply/supplyList?title="+$("#title").val();
				$(".secondType").removeClass("secondType");
			...
			success:function(data){
				...
				$.each(data,function(index,item){
					...
					if(rank == "pic"){
						...
						if(item.lowPrice=="undefined"||item.lowPrice==null){
							str += '面议';
						}else{
							str += item.lowPrice;
						}
						str +='</p><span class="font_1 info-div-tr">'+item.pname+''+item.cname+
						'</span><a onclick="goDetail('+item.id+')" class="font_1 info-div-mid" href="javascript:void(0)">'+item.title+'</a>'+
				        ...
				        if(item.details.length >75){
				         ...
				         str+='</dt>'+
				        '<dd>求购商<span onclick="goCom('+item.cid+')">'+item.yname +'</span><a>地区：'+item.pname+''+item.cname+'</a></dd></dl><ul class="rzh">';
					        if(item.youyiFlag == '1'){
					...
					if(pageNum >= 5){
						if(totlePage - parseInt(pageNum) >=4){
							for(var i = (parseInt(pageNum)-3);i<(parseInt(pageNum)+4);i++){
								if(pageNum == i){
									fanye+='<input id="page'+i+'" type="button" class="cur_fanye_btn" value="'+i+'"/>';
								}else{
    ...
    function showMess(str){
	 ...
	 function goComp(str){
		 window.location.href="${ctx}/homepage/"+str;
	 }
	 function goPic(str){
		 if(str != ""){
