...
<div class="main_right">
    ...
    <div class="tab_main">
        <div class="tab_title">
            <span onclick="parent.iframeGoToUrl('${ctx}/qiugou')">添加固废求购</span>
            <span onclick="parent.iframeGoToUrl('${ctx}/qiugou/qiugouPass?order=0')">已发布</span>
            <span class="m_r_tname" onclick="parent.iframeGoToUrl('${ctx}/qiugou/qiugouPass?order=1')">审核中</span>
        ...
        <form action="">
                    ...
                    <c:forEach items="${records }" var="record" varStatus="status">
                    	<tr>
                            ...
                            <c:if test="${order ge 0 }">
	                           		<a class="tr_manage_1" style="position:relative;" onclick="parent.iframeGoToUrl('${ctx}/qiugou?paramId=${record.id}&orderS=${order }')">&nbsp;<h6>修改</h6></a>
                            </c:if>	
			...
			<c:set var="actionUrl" value="${ctx}/qiugou/qiugouPass/"/>
...
<script>
    ...
    $(function(){
		...
		$(".tab_main_row :button").eq(0).click(function(){
			...
			location.href = '${ctx}/qiugou/qiugouPass?order=${order }&one='+parent+"&two="+child+"&title="+sousuoText+"&paginateFenye=${paginateFenye}";
		});
		//重置
		...
		/* $("#demandSecond").change(function(){
			conditon = $(this).val();
			if(conditon != 0){
				location.href = '${ctx}/qiugou/qiugouPass?order=${order }&conditon='+conditon;
			}
		}); */
		//分页
	...
	function jiazai(){
		...
		var parent  = $("#demandFirst option:selected").val();
		/* if(parent == 0){
			location.href = '${ctx}/qiugou/qiugouPass?order=${order }';
		}  */
		//加载二级
		$.ajax({
			type : "POST",
			...
			success : function(data) { 
				if(data == "ok"){
					...
					setTimeout(function(){
						...
						location.href = '${ctx}/qiugou/qiugouPass?order=${order }'+aaa;
	            	},1500);
				}
			}
		});
