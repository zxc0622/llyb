...
public class PoiKit {
    ...
    public HSSFWorkbook export() {
        ...
        for (int i = 0, len = data.size(); i < len; i++) {
            ...
            if (obj instanceof Map) {
                ...
                processAsRecord(columns,headers, row, obj);
            }
        }
        return wb;
    }
    ...
    private static void processAsRecord(String[] columns,String[] headers, HSSFRow row, Object obj) {
        HSSFCell cell;
        ...
        if (columns.length == 0) {// 未设置显示列，默认全部
            ...
            for(int i=0;i<headers.length;i++){
            	 cell = row.createCell(columnIndex);
                 cell.setCellValue(record.get(headers[i]) + "");
                 columnIndex++;
            }
            /*for (String key : keys) {
                cell = row.createCell(columnIndex);
